const firebaseConfig = {
    databaseURL: "https://bingo99-569c1-default-rtdb.firebaseio.com/"
};
...
function initReadySystem() {
    ...
    if (savedReadyKey) {
        ...
        readyPlayersRef.child(playerReadyKey).once('value', (snapshot) => {
            if (snapshot.exists()) {
                ...
                // Ya no existe en Firebase, limpiar local
    ...
    // Escuchar cambios en el contador - CORREGIDO PARA NUEVO SORTEO
    readyPlayersRef.on('value', (snapshot) => {
        const players = snapshot.val() || {};
        const count = Object.keys(players).length;
        updateReadyCounter(count);

        // IMPORTANTE: Verificar si el jugador actual fue eliminado (nuevo sorteo)
        if (isPlayerReady && playerReadyKey && !players[playerReadyKey]) {
            // El jugador fue eliminado de Firebase - resetear estado local
            console.log('🔄 Jugador eliminado de ready_players - Resetear estado');
            localStorage.removeItem('playerReadyKey');
            playerReadyKey = null;
            isPlayerReady = false;
            updateReadyButtonUI();
            showNotification('🔄 Nuevo sorteo iniciado. Presiona "Listo" para participar.', 'info');
        }
    });

    // Escuchar señal de nuevo sorteo para resetear estado (backup)
    db.ref('nuevo_sorteo').on('value', (snapshot) => {
        const data = snapshot.val();
        if (data && data.timestamp) {
            // Nuevo sorteo iniciado - resetear estado del jugador
...
function resetPlayerReadyState() {
    console.log('🔄 Ejecutando resetPlayerReadyState');

    // Quitar al jugador de la lista de listos en Firebase
    if (playerReadyKey) {
    ...
    console.log('🔄 Estado del jugador reseteado completamente');
}
...
function updateReadyButtonUI() {
    console.log('🎨 Actualizando UI del botón. isPlayerReady:', isPlayerReady);
    ...
    if (isPlayerReady) {
        ...
        readyBtn.style.background = '';  // Resetear a estilo por defecto
        readyBtn.style.borderColor = '';
        readyBtn.style.animation = '';

        // Asegurar que el botón vuelva a su estilo original (no verde)
        readyBtn.style.background = 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)';
    }
}
